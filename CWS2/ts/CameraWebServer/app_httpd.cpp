...
{
  printf("=== TANK HANDLER CALLED - START ===\n");
  log_i("Tank handler called - START");
  //serialWrite("Tank handler called - START");
  ...
  log_i("Variables initialized successfully");
  //serialWrite("Variables initialized successfully");
  ...
  log_i("Query string length: %d", buf_len);
  //serialWrite("Query string length: " + String(buf_len));

  // More than the single null terminator
  if (buf_len > 1)
  {
    printf("Query string found, processing...\n");
    log_i("Query string found, processing...");
    //serialWrite("Query string found, processing...");
    
    printf("About to call httpd_req_get_url_query_str...\n");
    // Check for query string and copy to buffer
    esp_err_t query_result = httpd_req_get_url_query_str(req, buf, buf_len);
    printf("httpd_req_get_url_query_str returned: %d\n", query_result);
    
    if (query_result == ESP_OK)
    {
      printf("Query string extracted: %s\n", buf);
      log_i("Query string extracted: %s", buf);
      //serialWrite("Query string extracted: " + String(buf));
      
      printf("About to call httpd_query_key_value...\n");
      // Check for value of expected key `dir`
      // Params: query from, key, value out, size of value
      esp_err_t key_result = httpd_query_key_value(buf, "dir", direction, sizeof(direction));
      printf("httpd_query_key_value returned: %d\n", key_result);
      
      if (key_result == ESP_OK)
      {
        printf("Direction parameter found: %s\n", direction);
        log_i("Direction parameter found: %s", direction);
        //serialWrite("Direction parameter found: " + String(direction));
        
        printf("About to call setSpeed(128, 128)...\n");
        setSpeed(128 , 128); // Reset speed to default before applying new speeds
        printf("setSpeed(128, 128) completed\n");
        ...
        if (strcmp(direction, "fw") == 0)
        {
          ...
          log_i("Executing forward command");
          printf("About to call fwd()...\n");
          fwd();
          printf("Forward command completed\n");
          log_i("Forward command completed");
        }
        else if (strcmp(direction, "bw") == 0)
        {
          printf("Executing backward command\n");
          log_i("Executing backward command");
          printf("About to call bck()...\n");
          bck();
          printf("Backward command completed\n");
          log_i("Backward command completed");
        }
        else if (strcmp(direction, "lt") == 0)
        {
          printf("Executing left command\n");
          log_i("Executing left command");
          printf("About to call lft()...\n");
          lft();
          printf("Left command completed\n");
          log_i("Left command completed");
        }
        else if (strcmp(direction, "rt") == 0)
        {
          printf("Executing right command\n");
          log_i("Executing right command");
          printf("About to call rgt()...\n");
          rgt();
          printf("Right command completed\n");
          log_i("Right command completed");
        }
        else if (strcmp(direction, "st") == 0)
        {
          printf("Executing stop command\n");
          log_i("Executing stop command");
          printf("About to call setSpeed(0, 0)...\n");
          // Stop. Set speed to 0
          setSpeed(0, 0);
          printf("Stop command completed\n");
          log_i("Stop command completed");
        }
        else
        {
          printf("Invalid direction parameter: %s\n", direction);
          log_e("Invalid direction parameter: %s", direction);
      ...
      {
        printf("ERROR: Missing dir parameter in query string (key_result: %d)\n", key_result);
        log_e("Missing dir parameter in query string");
    ...
    {
      printf("ERROR: Failed to extract query string (query_result: %d)\n", query_result);
      log_e("Failed to extract query string");
  ...
  {
    printf("ERROR: No query parameters found\n");
    log_e("No query parameters found");
  ...
  printf("Preparing successful response\n");
  log_i("Preparing successful response");
  // Successful response
  httpd_resp_set_type(req, "application/json");
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
  const char *resp_str = "{\"status\":\"ok\"";
  httpd_resp_send(req, resp_str, HTTPD_RESP_USE_STRLEN);
  printf("=== TANK HANDLER COMPLETED SUCCESSFULLY - END ===\n");
  log_i("Tank handler completed successfully - END");
  return ESP_OK;
}
